    ...
    private fun drawObjects(canvas: Canvas) {
        ...
        GameManager.towerList.forEach { tower ->
            ...
            if(tower.isShooting && GameManager.waveActive) {
                draw(canvas, BitmapPreloader.weaponAnimsArray[tower.towerLevel][tower.type]!!.nextFrame(tower.orientation), Vector2D(tower.position.x + tower.rotationCorrection, tower.position.y + tower.weaponOffset + tower.rotationCorrection))
            } else {
